...
import { queueProvider, type QueueMessage } from '../core/queue.js';
...
async function sweepPendingReinforcements(): Promise<void> {
  ...
  for (const row of pending.rows) {
    await processReinforcementEvent(row.id as string);
  }
}

async function processReinforcementEvent(eventId: string): Promise<void> {
  ...
  try {
    ...
    const lockAcquired = await withAdvisoryLock(LOCK_REINFORCEMENT, async (tx) => {
      ...
      for (const row of retrievals.rows) {
        ...
        const { reinforcementCount, cognitiveMomentum } = CognitionService.calculateReinforcement(
          node.reinforcementCount,
          Number(node.cognitiveMomentum),
          count
        );
