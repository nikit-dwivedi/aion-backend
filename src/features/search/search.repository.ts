...
export class SearchRepository {
  static async findSimilarMemories(queryEmbedding: number[], userId: string, limit = 5) {
    return await db
      .select({
        id: nodes.id,
  ...
  static async getMemoryConnections(memoryNodeIds: string[], userId: string) {
    if (memoryNodeIds.length === 0) return { rows: [] };
