...
Output ONLY raw JSON.`;
  ...
  static async getCognitionDashboard(userId: string) {
    ...
    const sentNotifs = Number((notificationsCountRes.rows[0] as any)?.count || 0);
    const requestedNotifs = Number((notifRequestsRes.rows[0] as any)?.count || 0);
    ...
    const totalRetrievals = Number((retrievalRes.rows[0] as any)?.total || 0);
    const successfulRetrievals = Number((retrievalRes.rows[0] as any)?.successful || 0);
    const retrievalSuccessRate = totalRetrievals > 0 ? (successfulRetrievals / totalRetrievals) : 1.0;

    const metrics: Record<string, { avgLatency: number; count: number }> = {};
    let totalLatency = 0;
    let latencyCount = 0;
    ...
    return {
      ...
      reinforcementCount: Number((reinforceRes.rows[0] as any)?.count || 0),
