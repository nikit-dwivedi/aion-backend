...
export class FocusService {
   ...
   */
  static async getFocusToday(userId: string) {
    ...
    const now = new Date();
    const updatedLoops: typeof userLoops = [];
    ...
    const actionItems = await db
      ...
      .where(and(
        ...
        eq(nodes.nodeType, 'action_item'),
        sql`COALESCE(metadata->>'status', 'pending') = 'pending'`
      ));
