...
async function sweepPendingEpisodes() {
  ...
  `);
    for (const row of pending.rows) {
        await processEpisodeEvent(row.id);
    }
}
...
async function processEpisodeEvent(eventId) {
    ...
    try {
      ...
      `);
        ...
        `);
            ...
            const memories = unclustered.rows.map((r) => ({
                id: r.id,
                content: r.content,
            ...
            for (let i = 0; i < memories.length; i++) {
                ...
                const entityOverlap = mem.metadata?.entities && lastMem.metadata?.entities
                    ? mem.metadata.entities.filter((e) => lastMem.metadata.entities.includes(e)).length
                    : 0;
            ...
            for (const cluster of clusters) {
                const matchingIds = cluster.map((m) => m.id);
          ...
          AND e.target_node_id IN (${sql.raw(matchingIds.map((id) => `'${id}'`).join(','))})
          LIMIT 1
        `);
                if (preCheck.rows.length > 0) {
                    continue; // Part of this cluster is already grouped
                }
                const listText = cluster.map((m, idx) => `[ID: ${m.id}] Thought ${idx + 1}: ${m.content} (${m.createdAt.toLocaleTimeString()})`).join('\n');
                const prompt = `
...
//# sourceMappingURL=episode_worker.js.map