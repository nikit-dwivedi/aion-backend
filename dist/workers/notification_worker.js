...
async function sweepPendingNotifications() {
  ...
  `);
    for (const row of pendingEvents.rows) {
        await processNotificationEventById(row.id);
    }
}
async function processNotificationEventById(eventId) {
  ...
  `);
    ...
    const priority = eventRow.priority || 'normal';
    ...
    try {
      ...
      `);
            const recentCount = Number(recentNotifications.rows[0]?.count || 0);
