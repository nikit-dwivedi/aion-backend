...
const MAX_RETRIES = 5;
export function isEventReadyForRetry(createdAt, retryCount) {
    if (retryCount === 0)
        return true;
...
async function sweepPendingEvents() {
  ...
  `);
    ...
    `);
        await db.transaction(async (tx) => {
            ...
            if (!rawThoughtNode)
                throw new Error('Failed to insert raw thought node');
            let memoryNodeId;
            const topMatch = similarNodes.rows[0];
            if (topMatch && topMatch.similarity > 0.80) {
                memoryNodeId = topMatch.id;
                const mergePrompt = `
        ...
        `;
                ...
                const existingMeta = { ...(topMatch.metadata || {}) };
        ...
        `);
            ...
            for (const simNode of similarNodes.rows) {
                if (simNode.similarity > 0.85 && simNode.id !== memoryNodeId) {
                    await tx.insert(edges).values({
                        sourceNodeId: rawThoughtNode.id,
                        targetNodeId: simNode.id,
                        relationType: 'similar_to',
                        weight: simNode.similarity,
                    });
                }
            }
            // Dynamic Node Resolver
            const resolveNode = async (nodeType, content) => {
        ...
        `);
            ...
            const linkMemoryToProject = async (memId, projId) => {
...
//# sourceMappingURL=llm_extractor.js.map