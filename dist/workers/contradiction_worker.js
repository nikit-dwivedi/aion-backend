...
async function sweepPendingContradictions() {
  ...
  `);
    for (const row of pending.rows) {
        await processContradictionEvent(row.id);
    }
}
async function processContradictionEvent(eventId) {
    ...
    try {
      ...
      `);
            ...
            for (const row of contradictions.rows) {
                const srcNodeId = row.source_node_id;
                const tgtNodeId = row.target_node_id;
                const srcText = row.src_content;
                const tgtText = row.tgt_content;
...
//# sourceMappingURL=contradiction_worker.js.map