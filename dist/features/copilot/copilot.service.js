...
Output ONLY the raw JSON array, no markdown, no explanation.`;
        ...
        aiResponse = aiResponse.replace(/"sources"\s*:\s*\[([^\]]*)\]/g, (_match, inner) => {
        ...
        if (si !== -1 && ei !== -1)
            aiResponse = aiResponse.substring(si, ei + 1);
        let insights;
        try {
        ...
        const enriched = insights.map((ins) => {
            const sourceMemories = (ins.sources || []).map((idx) => {
    ...
    static async getInsights(userId) {
        ...
        return insightNodes.map(n => {
            try {
                return { id: n.id, ...JSON.parse(n.content), createdAt: n.createdAt };
            }
            catch {
                return { id: n.id, type: 'pattern', title: 'Insight', body: n.content, createdAt: n.createdAt };
            }
        });
    }
    static async getNudge(userId) {
        if (!llm.isConfigured)
            return null;
...
Output ONLY raw JSON without markdown formatting.`;
    ...
    static async chat(userId, message, conversationHistory) {
        ...
        const memCtx = recentMemories.map((m, i) => `${i + 1}. ${m.content}`).join('\n');
        const historyText = (conversationHistory || []).map((h) => `${h.role === 'user' ? 'User' : 'AION'}: ${h.content}`).join('\n');
