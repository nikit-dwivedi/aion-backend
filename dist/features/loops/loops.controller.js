...
export class LoopsController {
    ...
    static async resolveLoop(req, res) {
        const userId = req.userId;
        if (!userId)
            throw new AppError('Unauthorized', 401);
        ...
        const [updated] = await db
            ...
            .returning();
        if (!updated)
            throw new AppError('Loop not found or unauthorized', 404);
        res.json({
            success: true,
    ...
    static async archiveLoop(req, res) {
        const userId = req.userId;
        if (!userId)
            throw new AppError('Unauthorized', 401);
        const loopId = req.params.id;
    ...
    static async snoozeLoop(req, res) {
        const userId = req.userId;
        if (!userId)
            throw new AppError('Unauthorized', 401);
        const loopId = req.params.id;
...
//# sourceMappingURL=loops.controller.js.map