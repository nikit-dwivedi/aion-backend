...
export class DeepDiveService {
    async getThoughtHistory(userId, rawThoughtId) {
    ...
    async chatWithThought(userId, rawThoughtId, message) {
        // 1. Verify thought belongs to user
        const thoughtQuery = await db.execute(sql `
      SELECT id, content FROM nodes 
      WHERE id = ${rawThoughtId} AND user_id = ${userId} AND node_type = 'raw_thought'
    ...
    `);
      ...
      ORDER BY n.created_at ASC
    `);
        let chatHistory = historyQuery.rows.map(r => `${r.metadata?.role === 'user' ? 'User' : 'AION'}: ${r.content}`).join('\n');
        ...
        await db.transaction(async (tx) => {
            ...
            if (summaryNode && summaryNode.content !== parsed.new_summary) {
                ...
                await tx.update(nodes)
                    ...
                    .where(eq(nodes.id, summaryNode.id));
...
//# sourceMappingURL=deepdive.service.js.map