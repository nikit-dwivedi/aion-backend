...
export class GraphRepository {
    ...
    static async getEdges(nodeIds, userId) {
        if (nodeIds.length === 0)
            return [];
     ...
     */
    static async getNeighborhood(seedNodeId, userId, maxDepth = 2, maxEdgesPerNode = 50) {
     ...
     */
    static async getEdgesBounded(nodeIds, userId, maxEdgesPerSource = 15) {
        if (nodeIds.length === 0)
            return [];
        const result = await db.execute(sql `
