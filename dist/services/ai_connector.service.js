...
function getTaskPriority(options) {
    if (options.subsystem === 'extractor')
        return 5;
    if (options.subsystem === 'orchestration' || options.subsystem === 'research')
        return 3;
    if (options.subsystem === 'clustering' || options.subsystem === 'episode' || options.subsystem === 'contradiction')
        return 1;
    const prio = (options.priority || '').toLowerCase();
    if (prio === 'critical' || prio === 'urgent')
        return 5;
    if (prio === 'important' || prio === 'normal' || prio === 'high')
        return 3;
    if (prio === 'background' || prio === 'low_priority_evolution' || prio === 'low')
        return 1;
    return 3;
}
export class AIConnectorService {
    primaryProvider;
    providersOrder;
    inferenceQueue = new LLMInferenceQueue();
    // Clients
    geminiClient = null;
    openAIClient = null;
    ollamaClient = null;
     ...
     */
    async executeWithRetry(provider, fn, maxRetries = 3) {
        ...
        return this.inferenceQueue.enqueue(priorityWeight, async () => {
            ...
            for (const provider of providers) {
                try {
                    ...
                    const response = await this.executeWithRetry(provider, () => this.callProviderGenerate(provider, normalizedOptions));
    ...
    async embedContent(text) {
        ...
        for (const provider of this.providersOrder) {
            try {
                ...
                const vector = await this.executeWithRetry(provider, () => this.callProviderEmbed(provider, text));
    ...
    isProviderConfigured(provider) {
        ...
        if (provider === 'ollama')
            return true; // Local fallback usually active
        return false;
    }
    async callProviderGenerate(provider, options) {
        // 1. GEMINI
        if (provider === 'gemini') {
            let contents;
    ...
    async callProviderEmbed(provider, text) {
        // 1. GEMINI
        if (provider === 'gemini') {
...
//# sourceMappingURL=ai_connector.service.js.map