 ...
 */
const checkAndResetDailyQuotas = async (user) => {
 ...
 */
export const incrementQuotaUsage = async (userId, metrics) => {
    try {
        const updates = {};
        if (metrics.tokens) {
        ...
        if (metrics.research)
            updates.researchRequests = sql `research_requests + 1`;
        if (metrics.planning)
            updates.planningExecutions = sql `planning_executions + 1`;
        if (metrics.retrieval)
            updates.retrievalQueries = sql `retrieval_queries + 1`;
        if (metrics.insights)
            updates.insightGenerations = sql `insight_generations + 1`;
